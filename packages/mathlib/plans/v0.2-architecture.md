...
**Possible cause (theory, still needs more testing):** View3D not properly disposed when module is hot-replaced. Old Three.js resources linger.
...
2. Test hot reload by editing a demo file - should not crash
3. Add a test that creates and removes items - verify no memory leaks
4. Log `scene.items.size` before and after removing items - should decrease
