...
Only act when size actually changes:

```typescript
// In createResponsiveThreeSizer or equivalent
let lastWidth = 0
let lastHeight = 0
...
resizeObserver.observe(container, (entries) => {
  ...
  lastWidth = width
  lastHeight = height
  // ... proceed with resize logic
})
```

**Skip update for clean items:**

In reconciler, only call `renderer.update()` if item is actually dirty:
...
**Problem:** "Resizing the window messes up the view a lot. Almost unusable without refresh"

**Root causes:**
1. Resize callback may fire multiple times rapidly
2. Camera aspect ratio might not update correctly
3. Renderer size vs CSS size mismatch
...
**Problem:** "Hot reload after editing something creates a weird state and sometimes crashes the tab"

**Root cause:** View3D not properly disposed when module is hot-replaced. Old Three.js resources linger.

**Fix:** Add `dispose()` method to View3D:
...
dispose() {
  ...
  // Clear meshes
  for (const [id, mesh] of this.threeMeshes) {
    const renderer = getRenderer(mesh.kind)
    renderer.dispose?.(mesh, this.threeScene)
  }
...
2. **Bug fixes** - Dispose method and resize fix
3. **Item lifecycle** - Add scene.remove() 
4. **Performance quick wins** - Resize optimization, dirty check
5. **View3D refactor** - Extract modules (larger effort, can be incremental)
6. **Two-way communication** - Camera sync (after View3D refactor is cleaner)
...
2. Test resize by dragging browser window - should be smooth
3. Test hot reload by editing a demo file - should not crash
4. Add a test that creates and removes items - verify no memory leaks
5. Log `scene.items.size` before and after removing items - should decrease
