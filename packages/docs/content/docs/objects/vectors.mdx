...
A vector that rotates around the y-axis:
...
const v = scene.create("vector3d", {
  ...
  vector: scene.atom((get) => {
    const angle = get(t);
    return vec3(Math.cos(angle) * 3, 0, Math.sin(angle) * 3);
  }),
  color: "yellow",
  draggable: "none",
});
...
```typescript
const a = scene.atom(vec3(0, 0, 0));
const b = scene.atom(vec3(3, 2, 1));

scene.create("point3d", { coords: a, color: "red" });
scene.create("point3d", { coords: b, color: "blue" });
