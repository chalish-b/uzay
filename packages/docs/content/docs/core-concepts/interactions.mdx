---
title: Interactions
description: Drag, click, and hover events on scene objects
---
...
MathLib has a built-in interaction system that lets users drag, click, and hover over objects in the scene. Some objects (like points and vectors) come with default drag behavior, but you can also attach your own event handlers to any object.
...
The `draggable` property controls which axes an object can be dragged along:
...
You can listen for drag, click, and hover events on any object using `.on()`:
...
- **`"enter"`**: pointer enters the object
- **`"move"`**: pointer moves while still over the object
- **`"leave"`**: pointer leaves the object
...
|-------|------|-------------|
| `itemId` | `string` | ID of the object that received the event |
| `itemKind` | `string` | Type of the object (e.g. `"point3d"`) |
...
Every object has a `pointerEvents` property that controls whether it participates in the interaction system:

```typescript
// This grid won't block clicks on objects behind it
scene.create("grid3d", {
  pointerEvents: "none",
});
...
scene.create("sphere3d", {
  center: vec3(0, 0, 0),
  pointerEvents: "auto", // This is the default for most objects
});
```

- **`"auto"`**: the object can be clicked, hovered, and dragged (default for most objects)
- **`"none"`**: the object is invisible to the pointer and events pass through it

This is particularly useful for decorative objects like grids and axes that you don't want intercepting mouse events meant for the objects behind them.
