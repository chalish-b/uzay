...
**Items** are the visual elements in a scene: points, lines, vectors, parametric curves, surfaces, overlays, and more. Every item property is an atom, so items are reactive by default. They come in [3D](/docs/3d-items) and [2D](/docs/2d-items) families that share the same mechanics.
...
[Learn more about Items](/docs/core-concepts/items)
...
```typescript
const p = curvePoint(scene, { f: myFunc, t: 0.5 });
...
<Cards>
  ...
  <Card title="Items" href="/docs/core-concepts/items" description="Creating, updating, and removing items" />
