...
Some object properties expect functions, like `f` in parametric curves or surfaces. When a function's behavior depends on parameters, the best approach is to make those parameters atoms and derive the function from them:
