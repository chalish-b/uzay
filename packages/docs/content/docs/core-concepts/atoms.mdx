...
To learn more about atoms, check out [Jotai](https://jotai.org). The atom implementation in MathLib is a simple wrapper around it, which binds atoms to the scene's `store`.
...
// MathLib (bound to scene's store)
