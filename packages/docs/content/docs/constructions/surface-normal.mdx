...
A `surfaceNormal` draws the normal vector to a [surface](/docs/items/surfaces) at a given $(x, z)$ coordinate. It computes the cross product of the surface's partial derivatives numerically, so you don't need to provide a gradient. Useful for:
