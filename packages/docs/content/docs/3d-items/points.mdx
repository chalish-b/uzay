...
import PointsDemo from "@/components/demos/points-demo";
...
The pink point is a plain draggable `point3d` with an `overlay3d` label reading its coordinates. The teal point's position is a derived atom, so it follows every drag automatically.
...
Points support transparency via the `opacity` property (0 = fully transparent, 1 = fully opaque):
...
This is handy for de-emphasizing reference points without hiding them entirely.
