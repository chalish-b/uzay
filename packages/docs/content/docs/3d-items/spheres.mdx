...
The `sphere3d`'s `center` is one draggable `point3d` and its `radius` is a derived atom, the distance to the second point, so the sphere always passes through it. The slider drives the `opacity` atom, and the `overlay3d` reads the radius live.
