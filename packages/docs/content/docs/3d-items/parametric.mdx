...
Curves render as shaded 3D tubes by default. Set `style: "flat"` for an unshaded flat stroke that keeps a constant pixel width at any camera distance, and pair it with `dashed` for a dashed curve:

```typescript
scene.create("parametricfunction3d", {
  f: (t) => vec3(Math.cos(t) * 3, t * 0.25, Math.sin(t) * 3),
  tStart: 0,
  tEnd: Math.PI * 6,
  color: "cyan",
  thickness: 1.5,
...
The flat style is also much cheaper to render, which matters for dense curve families (lattices, field lines, images of grids under a map): a scene can hold hundreds of flat curves and update them live without stuttering. See [Lines](/docs/3d-items/lines#tube-or-flat) for a full comparison of the two styles.

## Opacity

Curves support transparency via the `opacity` property (0 = fully transparent, 1 = fully opaque):

```typescript
scene.create("parametricfunction3d", {
  f: (t) => vec3(Math.cos(t) * 3, Math.sin(t) * 3, t * 0.3),
  tStart: 0,
  tEnd: Math.PI * 6,
  color: "cyan",
...
This is handy for rendering dense families of guide curves faintly, keeping the main curves readable on top.
...
function findNearestTOnCurve(
  ...
  for (let i = 0; i < 8; i++) {
    const v = f(t).sub(ray.origin);

    // Component of v perpendicular to the ray
    const proj = v.dot(d);
    const perp = v.sub(d.scale(proj));

    // Numerical derivative f'(t)
    const EPS = 1e-5;
    const deriv = f(t + EPS).sub(f(t - EPS)).scale(1 / (2 * EPS));

    // Newton step: move t to reduce the perpendicular distance
    const g = deriv.dot(perp);
