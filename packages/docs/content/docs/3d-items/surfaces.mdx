...
import SurfacesDemo from "@/components/demos/surfaces-demo";
...
Surfaces are useful for visualizing functions of two variables, terrain, potential fields, probability distributions, and more. For surfaces that cannot be written as a heightfield, like cylinders, spheres, and tori, see [ParametricSurface3D](/docs/3d-items/parametric-surfaces).

<SurfacesDemo />

The ripples are a single `surface3d` whose `f` is a derived atom: it recomputes from the pink `point3d`'s coordinates, so dragging the source re-centers the whole wave. The checkbox flips the surface's `wireframe` atom to expose the underlying sample mesh.
