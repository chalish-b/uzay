...
The pink arrows are two draggable `vector3d` items chained tip to tail: $\vec{b}$'s `origin` is $\vec{a}$'s `vector` atom, so dragging $\vec{a}$ carries the chain along. The indigo arrow is the derived sum $\vec{a} + \vec{b}$, and the faint gray pair completes the parallelogram, with a copy of $\vec{b}$ at the origin showing that a vector is independent of where it is drawn.
...
Vectors support transparency via the `opacity` property (0 = fully transparent, 1 = fully opaque). The shaft and head fade together:
