...
import PlanesDemo from "@/components/demos/planes-demo";
...
The `plane3d`'s `point` and `normal` are derived atoms: the centroid of three draggable `point3d` handles, and the cross product of two triangle edges. The `vector3d` arrow shows the derived normal, an `overlay3d` reads it live, and the slider drives the shared `width`/`height` atom.
